...
class OidnTest {
    ...
    class TestLocator : LibraryLocator {
        ...
        override fun locate(library: Library): File = possibles
            .map { File(it, library.filename()) }
            .find { it.exists() }
            ?: throw FileNotFoundException("Cannot find ${library.filename()} in $possibles")
    }

    private val oidn = Oidn.init(locator())

    companion object {
        fun locator(): LibraryLocator = if ( System.getProperty("test.use-package-locator") != null ) {
            PackagedLibraryLocator("oidn")
        } else {
            TestLocator()
        }
    }
    ...
    @Test
    fun `library version`() {
        val newDevice: OidnDevice = oidn.newDevice(Oidn.DeviceType.DEVICE_TYPE_DEFAULT)
        val version = newDevice.use { device -> device.version() }
